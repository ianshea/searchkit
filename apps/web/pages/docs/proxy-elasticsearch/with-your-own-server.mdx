...
const transporterConfig: ConfigConnection = {
    host: "https://some-custom-endpoint/search/elastic",
    headers: {
...
### Backend
On the backend, this example uses Elastic's PHP SDK. Searchkit's requests POST to `${yourendpoint}/_msearch`

```php
// Assuming the route handler `https://some-custom-endpoint/api/search/_msearch`
